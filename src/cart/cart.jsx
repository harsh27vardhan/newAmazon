...
import { changeCartSelection, resetCart } from "../cart/cartSlice";
...
export default function Cart() {
    ...
    return (
        <div className="cart flex items-start gap-6 overflow-auto min-w-screen max-w-[100dvw] bg-[#e9eded] mt-[50px]">
            ...
            <div className="min-w-[642px] max-w-[1180px] products-list w-fit flex flex-col justify-center gap-3 p-4 bg-white">
              ...
              <div className="flex flex-col ">
                {cartData.map((product) => (
                  <CartProduct
                    key={product.product_id}  // Make sure to add the unique 'key' prop
                    data={product}
                    cartData={cartData}
                  // updateCart={updateCart}  // Uncomment if you need to pass the updateCart function
                />
                ))}
              </div>
            </div>
...
const CartProduct = ({ data, cartData }) => {
    const {
      product_name,
      img_link,
      discounted_price,
    ...
    return (
      <div className="cart-product flex gap-3 p-4 max-w-[1140px]">
        <input
          type="checkbox"
          checked={selected}
          onChange={() => dispatch(changeCartSelection(data))}
        />
